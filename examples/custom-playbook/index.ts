import { definePlaybook, type PlaybookContext } from 'parrat/core';
import { z } from 'zod';
...
export const rowCountCheckPlaybook = definePlaybook({
  ...
  async run(input: Input, ctx: PlaybookContext): Promise<Output> {
    ...
    const { executePlaybook } = await import('parrat/core');

    const result = await executePlaybook({
      playbookName: 'row-count-check',
      llm: ctx.llmClient,
      systemPrompt: `
