...
describe('saveReport', () => {
  ...
  it('creates reportsDir if it does not exist', async () => {
    ...
    await saveReport({ reportsDir, skillName: 'my-skill', html: '<html/>' });
  ...
  it('written file contents match the html argument', async () => {
    ...
    const result = await saveReport({ reportsDir: tempDir, skillName: 'my-skill', html });
    const contents = await readFile(result.filePath, 'utf8');
    expect(contents).toBe(html);
  });

  it('filename matches pattern <skill>-YYYYMMDD-HHmmss.html', async () => {
  ...
  it('two sequential calls with distinct mocked times produce distinct filenames', async () => {
    ...
    const r1 = await saveReport({ reportsDir: tempDir, skillName: 'my-skill', html: 'a' });

    vi.setSystemTime(new Date('2026-05-24T14:30:01.000Z'));
    const r2 = await saveReport({ reportsDir: tempDir, skillName: 'my-skill', html: 'b' });
