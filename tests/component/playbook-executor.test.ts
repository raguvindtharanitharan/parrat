...
import { executePlaybook } from '../../src/core/llm/playbook-executor.js';
...
describe('core/llm/playbook-executor', () => {
  beforeEach(() => {
  ...
  it('injects emit_findings into the tools list presented to Claude', async () => {
    ...
    await executePlaybook({ ...BASE_OPTIONS, playbookName: 'test-playbook', llm });
  ...
  it('returns the validated output on a single-turn happy path via emit_findings', async () => {
    ...
    const result = await executePlaybook({ ...BASE_OPTIONS, playbookName: 'test-playbook', llm });
  ...
  it('captures output and emits playbook_output_captured audit event', async () => {
    const mock = createMockMcpClient('dbt', [
      { name: 'list', description: 'list', inputSchema: {} },
    ]);
    vi.mocked(connectMcpClient).mockResolvedValue(mock);
    ...
    const captureEvent = events.find((e) => e.type === 'playbook_output_captured');
  ...
  it('throws MaxTurnsExceededError when budget is hit without emit_findings', async () => {
    ...
    await expect(
      executePlaybook({ ...BASE_OPTIONS, playbookName: 'budget-test', llm, maxTurns: 2 }),
  ...
  it('throws MaxTurnsExceededError on end_turn without a prior emit_findings call', async () => {
    ...
    await expect(
      executePlaybook({ ...BASE_OPTIONS, playbookName: 'no-emit-test', llm }),
    ).rejects.toThrow(MaxTurnsExceededError);
  });
