...
import { lineageAnalysisPlaybook } from '../../src/playbooks/lineage-analysis/index.js';
import { BASE_PROMPT } from '../../src/playbooks/lineage-analysis/prompt.js';

describe('playbooks/lineage-analysis', () => {
  const baseCtx = {
  ...
  it('has the expected name and kind', () => {
    expect(lineageAnalysisPlaybook.name).toBe('lineage-analysis');
    expect(lineageAnalysisPlaybook.kind).toBe('investigation');
  });

  it('declares the expected dbt tool allowlist', () => {
    const dbt = lineageAnalysisPlaybook.mcpServers?.dbt;
