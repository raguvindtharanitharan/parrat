...
import { metricDropRcaPlaybook } from '../../src/playbooks/metric-drop-rca/index.js';
import { BASE_PROMPT } from '../../src/playbooks/metric-drop-rca/prompt.js';

describe('playbooks/metric-drop-rca', () => {
  const baseCtx = {
  ...
  it('has the expected name and kind', () => {
    expect(metricDropRcaPlaybook.name).toBe('metric-drop-rca');
    expect(metricDropRcaPlaybook.kind).toBe('investigation');
  });

  it('declares the expected dbt tool allowlist', () => {
    const dbt = metricDropRcaPlaybook.mcpServers?.dbt;
