...
/**
 * Per-Playbook MCP server declaration: the server config + the bare tool names
 * the Playbook allows Claude to invoke.
 */
...
export interface PlaybookExecutorOptions<TOutputSchema extends ZodTypeAny> {
  playbookName: string;
  llm: LlmClient;
  systemPrompt: string;
  userMessage: string;
...
export interface PlaybookExecutorResult<TOutput> {
  output: TOutput;
  totalTurns: number;
  inputTokens: number;
  outputTokens: number;
...
/**
 * Orchestrates the agentic execution of a Parrat Playbook:
 ...
 *   4. Emit claude_call audit events per turn and playbook_output_captured on emit_findings
 *   5. Close all MCP servers in try/finally regardless of outcome
 *
 * The Playbook's run() function calls this once and receives the validated output.
 * All MCP lifecycle, Claude conversation management, audit emission, and turn
 * budgeting happen here — the Playbook itself has no knowledge of any of it.
 */
export async function executePlaybook<TOutputSchema extends ZodTypeAny>(
  options: PlaybookExecutorOptions<TOutputSchema>,
): Promise<PlaybookExecutorResult<z.infer<TOutputSchema>>> {
  ...
  // Input schema is derived from the Playbook's outputSchema so the two stay in sync.
  ...
  try {
    ...
    for (let turn = 0; turn < options.maxTurns; turn++) {
      ...
      if (response.stop_reason === 'end_turn') {
        ...
        throw new MaxTurnsExceededError(options.playbookName, options.maxTurns);
      }
      ...
      throw new Error(
        `Unexpected stop_reason '${response.stop_reason}' from Claude in playbook '${options.playbookName}'`,
      );
    }
